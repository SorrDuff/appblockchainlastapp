...
contract MyContractBlockchain {
  ...
  function getAllUsers() public view returns(User[] memory) {
    ...
    function createJoinCode() private view returns (string memory) {
    uint256 random = uint256(keccak256(abi.encodePacked(block.timestamp, msg.sender)));
    bytes32 hexRandom = bytes32(random);
    return substring(bytes32ToString(hexRandom), 0, 6);
}
    ...
    function createClass(string memory _className) public {
        string memory joinCode = createJoinCode();
        classes.push(Class({
